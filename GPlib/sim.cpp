...
double computeDiffCoefOut( std::ofstream & output ,  std::ifstream & input , int comp_step , int start_step , int end_step , AtomList atom_list , AllTypeLUT lut_list , Cell cell )
{
  ...
  while( readStepXYZfast( input , atom_list , lut_list, true, true ) )
	...
	{
	  std::vector<double> x = backIn( difference( backIn( atom_list.x, cell.a ) , x0 ), cell.a ) ;
	  std::vector<double> y = backIn( difference( backIn( atom_list.y, cell.b ) , y0 ), cell.b );
	  std::vector<double> z = backIn( difference( backIn( atom_list.z, cell.c ) , z0 ), cell.c );
	  std::vector<double> r = square( squaroot( addVector( addVector( square( x ), square( y ) ), square( z ) ) ) );
