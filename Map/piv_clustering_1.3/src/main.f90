...
!      endif
    elseif (in_filetype.eq.2) then ! ----- pdb
      ...
      do
        ...
        if (ios/=0) exit
        if (line(1:6).eq."CRYST1") n_cryst1=n_cryst1+1
        if (line(1:5).eq."MODEL") n_steps=n_steps+1
        if (line(1:4).eq."ATOM".and.n_steps.eq.1) n_atoms=n_atoms+1
      enddo
...
!    call MPI_Bcast(atom_positions,N_steps*N_atoms*N_dim,MPI_double_precision,0,MPI_COMM_WORLD,mpierror)
      ...
      if(mpirank.eq.0) then
        ...
        !   print'(a)', 'WARNING: You probably are going to run out of memory... Good luck though !'
        ! endif  
