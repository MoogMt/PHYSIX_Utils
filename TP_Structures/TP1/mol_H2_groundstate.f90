...
real(8) :: rho(2), vol, vkin(2), vpot(2), pot, overl, coul, reson, etot(2)
...
open(10,file='mol_gs.input') ; read(10,nml=input) ; close(10)
open(8,file='energies_mol_gs.out')
open(7,file='density_3D_mol_gs.out')
...
n = 1
allocate(rad(0:n-1,0:np), c(0:n-1), psi(2,-nx:nx,-ny:ny,-nz:nz))
psi = 0._8
rad = 0._8
do l = 0, n-1
...
do ix = -nx, nx-1
  ...
  do iy = -ny, ny-1
    ...
    do iz = -nz, nz-1
      ...
      i = nint(rleft/dr)
      if(i.le.np) then
        psileft = rad(0,i)*s_harm(0,0,0._8,0._8)
      endif
      rright = sqrt((x-rmol/2)**2 + y**2 + z**2)
      i = nint(rright/dr)
      if(i.le.np) then
        psiright = rad(0,i)*s_harm(0,0,0._8,0._8)
      endif
      ...
      overl = overl + psileft*psiright * (dx**3)
      coul = coul - (dx**3) * psileft**2 * (0*pot(rleft,dx)+pot(rright,dx))
      reson = reson - (dx**3) * psileft*psiright * (0*pot(rleft,dx)+pot(rright,dx))
    enddo
if(imol.eq.idens) write(7,'(2f8.4,2f12.6)') x,y,psi(1,ix,iy,0),psi(2,ix,iy,0)
  enddo
...
write(8,'(f6.3,14f10.4)') rmol, rho, vpot, vkin, etot/rho+2/rmol, -1+2/rmol-(reson+coul)/(1+overl),   &
                                     -1+2/rmol+(reson-coul)/(1-overl), overl, coul, reson
write(6,'(f6.3,9f10.4)') rmol, rho, vpot, vkin, etot/rho+2/rmol
enddo
