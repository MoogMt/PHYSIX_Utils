...
function makeHistAngle( data::Array{T1,2}, nb_box_hist::T2 ) where { T1 <: Real, T2 <: Int }
    ...
    for i_box=1:nb_box
        hist_[i_box] = hist_[i_box]/abs(sin( conversion.degr2rad*(i_box*delta_+min_)))
    end
    hist_ /= sum(hist_[:])
    return hist_, delta_, min_
end
...
function makeHistAngleCO2( data::Array{T1,2}) where { T1 <: Real }
    min_ = 0
    max_ = 360
    nb_box = 361
    delta_ = 1
    hist_ = zeros(Real, nb_box )
    for i=1:size(data[:,1])[1]
        hist_[ round(Int, (data[i,1]- min_ )/delta_ +1 )  ] += 1
    end
    for i_box=1:178
        hist_[ i_box ] = hist_[i_box]/abs(sin( conversion.degr2rad*(i_box*delta_+min_)))
        hist_[ nb_box - i_box ] = hist_[ i_box ]
    end
    for i_box=179:180
        hist_[ i_box ] = hist_[178]
        hist_[ nb_box - i_box ] = hist_[ i_box ]
    end
    hist_ /= sum(hist_[:])
    return hist_, delta_, min_
...
function writeHist( file_out::T1, hist_avg::Vector{T2}, delta_::T3, min_::T4 ) where { T1 <: AbstractString, T2 <: Real, T3 <: Real, T4 <: Real }
    ...
    for i_box=1:nb_box
        Base.write( handle_out, string( (i_box-1)*delta_+min_, " ", hist_avg[ i_box ], "\n" ) )
    end
    close( handle_out )
    return true
end
        ...
        for step=1:nb_step
            ...
            for carbon=1:nbC
                ...
                elseif nb_neighbors == 2
                    ...
                    angleCO2 = geom.angleAlKash( a, b, c )
                    write( handle_angle_C2_X, string( angleCO2, " ", step, "\n" ) )
                    if check
                        write( handle_C2_Y, string( distances[ index[ 2 ] ], " ", step, "\n") ) # neigh +1 because we ignore the 0
                        write( handle_angle_C2_Y, string( angleCO2, " ", step, "\n" ) )
                    end
